   ...
   ## Prepare SCF-Files
	...
	if [[ ${sub_ok} == "true" ]] ; then #Submittierung  
        ...
        if [ "${oniom_ok}" == "true" ]; then
            ...
            sub-myg09-lambda_in_moria.sh ${monomerA} ${monomerB} ${foldername} ${zieldatei} oniom true ./${run_lambda_in##*Oniom/} 
            echo "sub-myg09-lambda_in_moria.sh ${monomerA} ${monomerB} ${foldername} ${zieldatei} oniom true ./${run_lambda_in##*Oniom/} "
        else 
            if [ -e ./lambda_in/ ] ; then
                cd ./lambda_in/
